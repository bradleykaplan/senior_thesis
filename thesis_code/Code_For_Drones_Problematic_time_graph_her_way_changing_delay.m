...
step = .01;
totalinstance=50;
for TimeToPark=0.1:0.1:20
    TimeFromPark=TimeToPark;
    TimeToPark
for instance=1:totalinstance
    ...
    TimeProbStations=getTimeProbStations(NumCarsPerStationOccupied,NumCarsPerStationReserved,Nspots,time_vect,Tfinal);
    TimeAverageProb(instance)=TimeProbStations/Tfinal;
...
meansTimeAverageProb(round(TimeToPark*10))=mean(TimeAverageProb)/Nstations;
musarray(round(TimeToPark*10))=TimeToPark;
end
%genXYPlot(musarray,meansTimeAverageProb,'plot',Tfinal,step)
figure;plot(musarray,meansTimeAverageProb,'o-');xlim([0,20])
...
function TimeProbStations = getTimeProbStations(NumCarsPerStationOccupied,NumCarsPerStationReserved,Nspots,time_vect,Tfinal)
dimension2 = size(NumCarsPerStationOccupied);
TimeProbStations = 0;
time_vect(end+1)=Tfinal;
for i = 1:dimension2(2)
    ...
    for j=1:dimension2(1)
        %if alreadycounted==0
            if NumCarsPerStationOccupied(j,i)+NumCarsPerStationReserved(j,i)==Nspots
                TimeProbStations = TimeProbStations+time_vect(i+1)-time_vect(i);
                %alreadycounted=1;
            end
        %end
    end
